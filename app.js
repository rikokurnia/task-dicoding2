const localTotalVictoryField = document.getElementById('local-total-victory-field');
  ...
  //remove and configuration start from first
  ...
  //event when storage was founded
  window.addEventListener('load', function () {
    ...
    // show store value on localstorage and sessionstorage
    sessionUserAttemptsField.innerText = sessionStorage.getItem(sessionUserAttemptsKey);
    localMaximumAttemptField.innerText = localStorage.getItem(localMaximumAttemptsKey);
    localTotalVictoryField.innerText = localStorage.getItem(localTotalVictoryKey)
  });

// function tombol "bermain"
  playButton.addEventListener('click', function () {
    sessionStorage.setItem(sessionAnswerKey, getAnswer()); //take a number from answer
    sessionStorage.setItem(sessionUserIsPlayingKey, true);
    beforeGameDisplay.setAttribute('hidden', true); //button 1,2,3 hidden
    duringGameDisplay.removeAttribute('hidden'); //button bermain will be hidden after button 1,2,3 had triggered
  });
    //for button 1 and if the length till to 3, show then answer
  answerButton1.addEventListener('click', function () {
    ...
    //for button 2 and if the length till to 3, show then answer
  answerButton2.addEventListener('click', function () {
    ...
    //for button 3 and if the length till to 3, show then answer
...
//for checking if the answer had corrected or wrong    
  function checkAnswer(userGuess) {
    const answer = sessionStorage.getItem(sessionAnswerKey);
    if (userGuess == answer) { //take the input if there correct
      duringGameDisplay.setAttribute('hidden', true);
      afterGameDisplay.removeAttribute('hidden');
      sessionTrueAnswerField.innerText = answer;
      updateScore();
    } else { //count the input whom wrong
...
//   for update score game in the form 'Local stats'
  function updateScore() {
    ...
    if (sessionAttemptsValue > localAttemptsValue) { //if stats tebakan salah more over than before it, it will be added as a new result
...
//  rmeove item storage for button "hapus semua data"
